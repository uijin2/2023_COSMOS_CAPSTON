...
from COSMOS.Calculation import ValueChecker
...
class Tello8890Sensor(Sensor):
    
    #=====Tello8890Sensor의 인스턴스를 생성시 실행될 함수=====
    def __init__(self, main):
        self.__printc("생성")
        self.__stop_event = main.stop_event
        self.__main = main
        self.__planner = main.planner
        self.__socket = main.socket8890
    ...
    def take_data_from_sensor(self): 
        ...
        data:bytes = self.__socket.recv(1518)
    ...
    def save_to_planner(self, info: str):
        ...
        if ValueChecker.is_tof_val(info):
            #ToF 값은 (int) 100 형태로 들어온다
            info = ValueChanger.change_mm_to_cm(int(info))
            if info > 60:
                info = 1000
            self.__planner.set_info_8890Sensor_tof(info)
            
        else: #cmd return 값이면
            self.__planner.set_info_8889Sensor_cmd(info)
            print("[Tello8890Sensor]",info)
